...
namespace PowerPosition
{
    public class PowerPositionService(ILogger<PowerPositionService> logger, IOptions<Settings> settings, PowerService powerService) : IPowerPositionService
    {
        ...
        private readonly PowerService _powerServiceClient = powerService;
        ...
        public async Task GenerateAsync()
        {
            ...
            try
            {
                ...
                {
                    _logger.LogWarning("No trades were returned for {TradeDate:yyyy-MM-dd}. CSV will still be generated with zero volumes.", tradeDate);
                }
                else
                {
                    _logger.LogInformation("Fetched {TradeCount} trades successfully for {TradeDate:yyyy-MM-dd}.", powerTrades.Count(), tradeDate);
                }

                var allTradePeriods = powerTrades.SelectMany(t => t.Periods).ToList();
                _logger.LogDebug("Flattened all trade periods: total {PeriodCount} records before aggregation.", allTradePeriods.Count);
